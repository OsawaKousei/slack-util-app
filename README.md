...
GAS の時間ベースのトリガーを使用することで、特定の機能を定期的に実行できます。例えば、`/mail` コマンドや `/slack-archive` コマンドを毎日自動で実行することが可能です。

1.  GAS エディタで「トリガー」（時計のアイコン）を開きます。
2.  「トリガーを追加」をクリックします。
3.  以下の項目を設定します：
    - **実行する関数を選択**: `archiveSlackMessages` や、`postRecentEmails`
    - **実行するデプロイを選択**: `Head`
    - **イベントのソースを選択**: `時間主導型`
    - **時間ベースのトリガーのタイプを選択**: `日付ベースのタイマー` や `時間ベースのタイマー`
    - **時刻を選択**: 実行したい時間帯
4.  「保存」をクリックします。

**注意**: 定期実行用の関数は `main.ts` で `global` にエクスポートされている必要があります。
...
3. **本番環境でのアクセス制限**
   ...
   - Web アプリとしてデプロイする際、「アクセスできるユーザー」を「自分のみ」に設定します。
   - これにより、スラッシュコマンド経由での外部からのアクセスを遮断し、時間トリガーによる定期実行のみで運用できます。セキュリティを重視する場合に有効な手段です。
